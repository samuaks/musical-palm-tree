...
import { Music, Play, Pause, SkipBack, SkipForward, VolumeX, Volume2, Volume1, Video } from 'lucide-react'
...
export function PlayerBar({ track, onNext, onPrev }: PlayerBarProps) {
  ...
  const { height, setHeight, startDrag } = useResizable({ defaultHeight: MIN_HEIGHT, min: MIN_HEIGHT , maxFromViewport: 120})

  const { playing, duration, currentTime, toggle, seek, volume, changeVolume, toggleMute, isVideo } =
  ...
  const showVideo = track && isVideo


  useEffect(() => {
    if (!track) return
    generateWaveform(track.path)
    if (videoExts.includes(track.ext.toLowerCase()) && height < 300) {
      setHeight(Math.floor(window.innerHeight * 0.7))
    } else if (!videoExts.includes(track.ext.toLowerCase()) && height > MIN_HEIGHT + 50) {
      setHeight(MIN_HEIGHT)
    }
  }, [track])

  const VolumeIcon = volume === 0 ? VolumeX : volume > 0.5 ? Volume2 : Volume1
  const videoHeight = showVideo ? Math.max(0, height - MIN_HEIGHT) : 0

  return (
    <div
      className="shrink-0 border-t border-slate-700 relative flex flex-col"
...
<div
  className={`w-full bg-slate-800 ${showVideo ? 'flex items-center justify-center' : 'hidden'}`}
